...
function tambah_data($KTP, $nama,$notelp, $tahun_masuk, $gambar){
  ...
  $query = "INSERT INTO data_karyawan (KTP, nama, notelp, tahun_masuk, gambar) VALUES
            ('$KTP','$nama','$notelp','$tahun_masuk','$gambar')";
  return run($query);
}

function edit_data($KTP, $nama, $notelp, $tahun_masuk, $gambar){
  $query = "UPDATE data_karyawan SET nama='$nama',notelp='$notelp', tahun_masuk='$tahun_masuk', gambar='$gambar'
            WHERE KTP='$KTP';";
  return run($query);
}

function edit_tanpa_gambar($KTP, $nama, $notelp, $tahun_masuk){
  $query = "UPDATE data_karyawan SET nama='$nama', notelp='$notelp', tahun_masuk='$tahun_masuk' 
            WHERE KTP='$KTP';";
  return run($query);
}
